...
def enable_coredump(info, root):
    ...
    if not sshClient.get_transport() or not sshClient.get_transport().is_active():
        logger.error("SSH connection is not established for enable coredump.")
        raise Exception("SSH connection is not established for enable coredump.")
    
    response = messagebox.askyesno("Confirmation","Enabling Coredumps require a reboot, do you want to proceed? ", parent=root)
    if response:
        channel = sshClient.invoke_shell() 
        channel.send(f'sudo rm -rf /etc/sysctl.d/50-disable-coredump.conf\n')
        time.sleep(0.1)
