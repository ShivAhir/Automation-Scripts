...
def formattedOutput(out, command):
    if command == 'cat /version':
        ...
        logger.info(f"This is the output of readMultiLines {readMultiLines(out)}")
...
def cardInfoD(info):
    ...
    if not sshClient.get_transport() or not sshClient.get_transport().is_active():
        ...
        raise Exception("SSH connection is not established for overview.")
    ...
    for command in commands:
        ...
        if results.get('mvdb-get cardinfo.license.mac_addr'):
            ...
            if name and sn_num and mac_addr:
                ...
                if not results.get('Decoded License'):
                    ...
                    if not results['Decoded License']:
                        logger.error("Required values for the final command are missing.")
            ...
            logger.info("Fomatted results in overview",formatted_results)
